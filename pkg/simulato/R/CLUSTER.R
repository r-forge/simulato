...
# state: 1:N - number of servers requested by ith oldest customer being served (if any), or zero; N+1: servers requested by fist customer in the queue; N+2: queue size (if any)
...
update_state <- function(sp,events){   # here we need to switch the state based on the triggering event
  ...
  if(events==N+1){ # arrival
    if(s[N+1]>0) s[N+2] <- s[N+2]+1 # to queue if already not empty
      ...
      if(sum(s[1:N])+customer_class<=N) s[which.min(s[1:N])]=customer_class # to server, if available
...
performance <- function(sp) {# what is the model performance measured at given point
  (sum(sp$state[1:(N+1)]>0)+sp$state[N+2]==0:15) # probabilities
  #sum(sp$state)==0
}