...
getPerformance.mmcluster <- function(m){# what is the model performance measured at given point
  ...
  m$state[m$gl$N+1]
  #sum(sp$state[1:(N+1)]>0)+sp$state[N+2] # number of customers in the system
  #cumsumServers = cumsum(m$state[1:m$gl$N])
  #nBusyServers = max(cumsumServers[which(cumsumServers<=m$gl$N)],0)
  #return(ifelse(m$state[m$gl$N+2] == 1, ifelse(nBusyServers == 0, m$gl$pIdle[1],  m$gl$чpLow[nBusyServers]), ifelse(nBusyServers == 0, m$gl$pIdle[2], m$gl$pHigh[nBusyServers])))
}
